...
export function LoginPage() {
  ...
  const { isGuest, enterGuest } = useGuestMode();
  const nav = useNavigate();
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (isGuest) nav("/", { replace: true });
    else if (!loading && user) nav("/", { replace: true });
  }, [user, loading, nav, isGuest]);
  ...
  return (
    <div className="ledger-bg flex h-screen items-center justify-center">
      <div className="paper-card w-full max-w-md p-10">
        ...
        <div className="mt-4 rounded-sm border border-dashed border-[color:var(--color-rule)] p-2 text-center text-[10px] text-[color:var(--color-ink-faint)]">
          Guest mode stores everything in your browser. Upload CSVs, run the pipeline, explore the review queue — no account needed.
        </div>
        <div className="mt-5 border-t border-[color:var(--color-rule)] pt-3 text-center text-[10px] text-[color:var(--color-ink-faint)]">
          Built by{" "}
          <a
            href="https://azoni.ai"
            target="_blank"
